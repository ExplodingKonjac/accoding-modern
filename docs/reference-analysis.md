...
原程序主循环每秒读榜且不断重绘；浏览器版支持 1/2/5/15/30/60 秒刷新，并在关闭或后台时暂停，动画使用 CSS，不增加接口轮询。
...
+1 模式按 `user.id` 和题目识别增量，使用 `wrong_count` 与最终判定区分失败及通过；待评测只增加首次尝试人数，不显示符号。排序变化不影响身份匹配。首次加载只建立基线，榜单回退时清理回放并直接同步。43 项自动测试通过；浏览器模拟验证 A 题只出现 `+1`、B 题只出现 `-`、C 题待评测没有符号，回放末人数与榜单一致。
