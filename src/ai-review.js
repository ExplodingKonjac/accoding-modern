function createAiReviewPanel(container,core,getContext,_readMetadata,options={}) {
  ...
  async function openDetail(id){
    ...
    try{
      ...
      const index=rows.findIndex(row=>String(row.submission_id)===String(id));previous.disabled=index<0||(index===0&&page===0);next.disabled=index<0||(index===rows.length-1&&(page+1)*size>=total);
      nav.append(previous,next,badge,close);head.append(title,nav);detail.replaceChildren(head);
  ...
  async function turn(direction,id){
    const index=rows.findIndex(row=>String(row.submission_id)===String(id));if(index<0)return;
    const target=rows[index+direction];if(target){await openDetail(target.submission_id);return;}
    const nextPage=page+direction;if(nextPage<0||nextPage>=Math.ceil(total/size))return;
    page=nextPage;await refresh();const next=direction>0?rows[0]:rows.at(-1);if(next)await openDetail(next.submission_id);
  }
