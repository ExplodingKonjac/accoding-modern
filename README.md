...
> 1.17.1 自动使用 OJ 源码读取权限同步；移除同步设置及冗余提示；无嫌疑提交移出核查列表。
...
- **按题查看通过提交**：选题查看本班赛内 AC，默认每人最新一条；可切换全部 AC、搜索同学，直接打开对应代码。
...
班级页读取当前比赛的规则结果，并统一展示「有AI嫌疑」「无AI嫌疑」。助教可更改结论、填写理由，或按提交 ID 查看规则未命中的代码。云端保留原始依据和全部修改历史，其他助教刷新即可查看。

前后版本使用红绿逐行 diff。完整历史先执行全部规则，再从命中结果中保留每人每题最多 1 份、每人最多 8 份，已取消模型核查。助教沿用 OJ 源码读取权限自动同步；改为无嫌疑后移出核查列表，理由及历史仍保存。

操作与存储说明见 [代码复核 1.17.1](docs/ai-review.md)。
