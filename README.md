...
> 1.14.1 代码复核免令牌查看，移除令牌设置；发布与修改继续要求签名。
...
详情显示疑似源码、原文证据和前一次提交链接。完整历史源码、批次队列与普通判断只保存在 Mac / ROG E 盘。查看结果不再需要令牌；启动、发布和修改任务仍由本机签名控制。页面自动读取最新 API Agent 运行，不再展示第二版、第三版和运行选择框。
